...
class ContinuousPendubotEnv(core.Env):

    """
    Pendubot is a 2-link pendulum with only the first joint actuated.
...
#         torque = self.AVAIL_TORQUE[a] # alp mod
        ...
        costs = .001 * (torque ** 2) + cos(s[0]) + cos(s[1] + s[0]) # alp mod
#         return (self._get_ob(), reward, terminal, {})
        return (self._get_ob(), - costs.item(), terminal, {}) # alp mod
...
#         return bool(-cos(s[0]) - cos(s[1] + s[0]) > 1.)# alp mod
    ...
    def _dsdt(self, s_augmented, t):
        ...
        # alp mod ddtheta1 and ddtheta2
        if self.book_or_nips == "nips":
            # the following line is consistent with the description in the
            # paper
            ddtheta1 = (a + d1 / d2 * phi2 - phi1) / \
                (m1 * lc1 ** 2 + I1 - d1 ** 2 / d2)
        else:
            # the following line is consistent with the java implementation and the
            # book
            ddtheta1 = (a + d1 / d2 * phi2 - m1 * l1 * lc1 * dtheta2 ** 2 * sin(theta1) - phi1) \
                / (m1 * lc1 ** 2 + I1 - d1 ** 2 / d2)
        ddtheta2 = -(d1 * ddtheta1 + phi2) / d2

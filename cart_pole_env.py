...
class Objective(object):
    ...
    def __init__(self):
        ...
        # self.Q = np.diag([10. ,10., 100.0, 10., 0.,  0.])
        self.Q = np.diag([100. ,20., 800.0, 60.0]) *0.1
        self.R = np.diag([.1])
        self.Rinv = np.linalg.inv(self.R)
        self.P = self.Q*0.
        self.P = np.diag([50. ,20., 400.0, 1.0])*0.01

        self.q = np.zeros(4)
        self.qf = np.zeros(4)

        self.target_x = np.array([0., 0., 0., 0.])
...
#         c = np.cos(x[2])
#         s = np.sin(x[2])
#         ldx = np.zeros(x.shape)
#         ldx[0] = 4. * (x[0]/2)**7#200. * x[0]
#         ldx[1] = 2.0*50. * x[1]
#         ldx[2] = 2.0*200.*(1-c)*s# + 0.2*s*c
#         ldx[3] = 0.2*x[3]
#         return ldx
    def ldx(self, x_t):
        x, x_dot, theta, theta_dot = x_t.copy()
        theta = wrap2pi(theta)
        ldx = np.zeros(x_t.shape)
#         ldx[0] = 4. * (x/2.0)**7#200. * x[0]
        ldx[0] = 100. * x
        ldx[1] = 2.0*50. * x_dot
        ldx[2] = 2.0 * 200. * theta
        ldx[3] = 0.1*theta_dot
